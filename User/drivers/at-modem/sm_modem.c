...
void sm_modem_init(sm_modem_t *modem,
    ...
    modem->driver = driver;
    modem->buff = NULL;
    modem->buff_length = buffer_size;
    if (buffer_size > MODEM_BUFF_SIZE)  modem->buff_length = MODEM_BUFF_SIZE;
    modem->lock = 0;
}
...
//	LOG_DBG(TAG, "cmd: %s ", cmd);
	...
	uint8_t modem_buffer[MODEM_BUFF_SIZE];
	modem->buff = modem_buffer;
    ...
    int32_t res = 0;
    while(1){
    	if (index_buff == MODEM_BUFF_SIZE) break;
